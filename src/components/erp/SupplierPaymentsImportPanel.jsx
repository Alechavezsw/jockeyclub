...
import {
  LISTA_BASE_PAGOS_FILENAME,
  LISTA_BASE_PAGOS_URL,
  SUPPLIER_PAYMENT_IMPORT_MODULES,
  SUPPLIER_PAYMENT_IMPORT_STATUS,
  buildSupplierPaymentImport,
  parsePagosSheetRows,
} from '../../domain/accounting/supplierPaymentImport';
...
export default function SupplierPaymentsImportPanel({
  suppliers = [],
  imports = [],
  onBack,
  onImportBatch,
  ...
  const [module, setModule] = useState('excel_manual');
  ...
  const processFile = async () => {
    ...
    try {
      ...
      const sheet = wb.Sheets.PAGOS || wb.Sheets[wb.SheetNames[0]];
      if (!sheet) throw new Error('No se encontró la hoja PAGOS en el archivo.');
      const aoa = XLSX.utils.sheet_to_json(sheet, { header: 1, defval: null });
      const rows = parsePagosSheetRows(aoa);
      if (!rows.length) {
        throw new Error('No hay pagos cargados en la hoja PAGOS (solo filas vacías).');
      }
      ...
      if (!built.payments.length) {
        throw new Error('Ninguna fila pudo importarse. Revisá proveedor y monto.');
      }
      ...
      setOk(
        `Importados ${built.batch.importedCount} pagos · ${formatCurrency(built.batch.totalAmount)}`
  ...
  return (
    <div className="fade-in supplier-pay-import" style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
      <section className="supplier-pay-import-block">
        <h4 className="supplier-pay-import-title">Importar pagos</h4>
        <div className="supplier-pay-import-form">
          <div className="supplier-pay-import-field">
            <label className="form-label" htmlFor="spi-module">Módulo</label>
            <select
              id="spi-module"
            ...
            >
              {Object.entries(SUPPLIER_PAYMENT_IMPORT_MODULES).map(([id, label]) => (
          ...
          <div className="supplier-pay-import-field">
            <label className="form-label" htmlFor="spi-file">Archivo</label>
            <input
              id="spi-file"
              ref={fileRef}
              type="file"
            ...
            <div style={{ marginTop: '0.65rem' }}>
              <a
                ...
                href={LISTA_BASE_PAGOS_URL}
                download={LISTA_BASE_PAGOS_FILENAME}
                style={{ display: 'inline-flex', alignItems: 'center', gap: 6 }}
              >
                <Download size={14} /> Descargar lista base
              </a>
              <p style={{ fontSize: '0.72rem', color: 'var(--text-muted)', margin: '0.45rem 0 0' }}>
                Plantilla LILA con hojas PAGOS, PROVEEDORES y FORMAS DE PAGO.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="supplier-pay-import-actions">
          ...
          <button
            ...
            style={{ display: 'inline-flex', alignItems: 'center', gap: 6 }}
          >
            {busy ? <Upload size={14} className="spin" /> : <FileSpreadsheet size={14} />}
              ...
              {imports.length === 0 ? (
                ...
                imports.map((item, idx) => (
                  <tr key={item.id}>
                    ...
                    <td>{item.moduleLabel || SUPPLIER_PAYMENT_IMPORT_MODULES[item.module] || item.module}</td>
                    <td>
                      <span className={`status-tag ${statusClass(item.status)}`}>
                        {SUPPLIER_PAYMENT_IMPORT_STATUS[item.status] || item.status}
                      </span>
                    </td>
